...
- (NSString *)cacheInternalPurchasePaymentPath {
    ...
    path = [NSString stringWithFormat:@"%@%@", path, @"/InternalPurchasePayment"];
        ...
        if (filesArray.count == 0) {
            ...
            for (NSString *name in filesArray) {
                if ([name hasSuffix:@".plist"]) {
                    ...
                    [resultArray addObject:payDic];
                }
            }
            if (completed) {
                completed(resultArray, nil);
            }
        }
    } else {
        completed(nil, error);
...
- (BOOL)savePaymentVoucher:(NSDictionary *)payDic {
    ...
    NSString *savedPath = [NSString stringWithFormat:@"%@/%@_%@.plist", path, k_applePayResult, fileName];
